...
export default function TesAnalogiVerbal({ auth, title, data }) {
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            <StartTesAnalogiVerbal title={title} data={data} user={auth.user} />
        </AuthenticatedLayout>
    );
}

function StartTesAnalogiVerbal({ title, data, user }) {
    let no = 1;
    ...
    const onClickSubmitAnswer = async () => {
        ...
        const finalData = {
            ...
            user_id: user.id,
            tav_data: finalTestData,
        };
        const postData = await axios.post(route("minatbakat.tav.scoring"), {
            data: finalData,
        });

        setTestData((prev) => prev.map((dt) => ({ ...dt, jawaban: null })));

        return router.post(route("minatbakat.tav.result"), {
            data: {
